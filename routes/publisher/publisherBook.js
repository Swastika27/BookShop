...
router.get('/', async (req, res) => {
    ...
    if(req.publisher == null) {
        console.log('inside publisher null');
...
router.get('/supply/delete/:id', async (req, res) => {
    if(req.publisher == null) {
        res.redirect('/publisher/login');
    }
    try {
        await DB_supply.deleteBookFromSupply(req.publisher.name, req.params.id);
        res.redirect(`/publisher/book/supplies`);
    } catch(err) {
        console.log(err);
    }
})
