...
app.get('/all_writers', async(req, res) => {
    ...
    console.log('single writer called');
    const data = await runQuery(`SELECT NAME, ID FROM WRITER ORDER BY NAME`);
    const writers = data.rows;
...
app.get('/all_writers/:id', async(req, res) => {
    ...
    const data = await runQuery(`SELECT * FROM WRITER WHERE id = ${id}`);

    console.log(data);
    res.render('single_writer', {writers: data.rows});
})